    ...
    public bool IsValidSudoku(char[][] board) 
    {
        ...
        for(int i = 0; i < 9; i++)
        {
            for(int j = 0; j < 9; j++)
            {
                ...
                if(boxDict.ContainsKey(GetBoxIndex(i,j)))//dict contains key
                {
                    if(IsElementInList(boxDict[GetBoxIndex(i,j)],result))
                    {
                        return false;
                    }
                    else
                    {
                        boxDict[GetBoxIndex(i,j)].Add(result);
                    }
                }
                else
                {
                    boxDict[GetBoxIndex(i,j)] = new List<int>{result};
                }
            }
        }
